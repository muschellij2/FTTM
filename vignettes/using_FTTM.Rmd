---
title: "Using The Functional Time Transformation Model"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Using Functional Time Transformation Model}
...
Here we show how to generate simulated data:
```{r, eval = FALSE}
###########Generating data##############################
set.seed(123)
n <- 500
...
We have wrapped this function into `FTTM::fttm_generate_data`:

```{r sim_data_tidy}
sim = fttm_generate_ph_data()
delta = sim$delta
T_obs = sim$T_obs
X = sim$data_scalar
Xs = sim$data_functional
S = sim$functional_domain
n = sim$n
m = length(S)
```
...
VCOVparfunc <- VCOVpar[(p1 + N0 + 1 + 1):(p1 + N0 + 1 + N1 + 1),
                       (p1 + N0 + 1 + 1):(p1 + N0 + 1 + N1 + 1)]
basismat <- NULL
for (k in 0:N1) {
  basismat <- cbind(basismat, dbeta((S), shape1 = (k + 1), shape2 = N1 - k +
                                      1))
}
...
###covariance matrix of parameters##
VCOVparfunc <- VCOVpar[(p1 + N0 + 1 + 1):(p1 + N0 + 1 + N1 + 1), (p1 + N0 +
                                                                    1 + 1):(p1 + N0 + 1 + N1 + 1)]
basismat <- NULL
for (k in 0:N1) {
  basismat <- cbind(basismat, dbeta((S), shape1 = (k + 1), shape2 = N1 - k +
